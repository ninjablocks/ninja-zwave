...
NinjaBlocks::NBZWInterpreter nbzwInterpreter;
...
//				nodeInfo = nodeInfo;		// placeholder for real action
				stringstream ss;
				ss << "\"h_" << (int)nodeInfo->m_homeId << "_n_" << (int)nodeInfo->m_nodeId << "\"";
				string s = ss.str();
				for( list<ValueID>::iterator it2 = nodeInfo->m_values.begin(); it2 != nodeInfo->m_values.end(); ++it2 )
				{
					...
					if (NinjaBlocks::NBZWInterpreter::isRecognised(v)) {
						nbzwInterpreter.valueChanged(s, v);
						break; //only send one recognised value (Work-around to only send one power)
					}
				}
			}
			break;
...
{
	nbzwInterpreter.connectWithModule();
